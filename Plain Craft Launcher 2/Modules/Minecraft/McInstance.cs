...
public class McInstance
{
    ...
    {
        PathInstance = (name.Contains(":") ? name : Path.Combine(GameFolderManager.CurrentFolder.Location, "versions", name)) + (name.EndsWithF(@"\") ? "" : @"\");
    }
    ...
    public string PathIndie
    {
        get
        {
            ...
            return Config.Instance.IndieV2[PathInstance] ? PathInstance : GameFolderManager.CurrentFolder.Location;
        }
    }
        ...
        get
        {
            ...
            try
            {
                ...
                if (JsonObject["patches"] is not null)
                    foreach (var patchNode in JsonObject["patches"].AsArray())
                    {
            ...
            if (field is null)
            {
                ...
                try
                {
                    ...
                    if (field.ContainsKey("patches") && !field.ContainsKey("time"))
                    {
                        ...
                        foreach (var SubjsonNode in field["patches"].AsArray())
                        {
                            var subjson = SubjsonNode.AsObject();
